...
add_filter( 'style_loader_src', function ( $src, $handle ) {
    ...
    $filename = get_theme_mod( 'css_bundle_version_number', 0 )
        ? picostrap_get_complete_css_filename()
        : picostrap_get_base_css_filename();
    $path = get_stylesheet_directory() . '/' . picostrap_get_css_optional_subfolder_name() . $filename;
    if ( is_file( $path ) ) {
...
add_filter( 'script_loader_src', function ( $src, $handle ) {
    if ( 'bootstrap5-childtheme' === $handle ) {
        $src = add_query_arg( 'ver', (string) filemtime( get_stylesheet_directory() . '/js/bootstrap.bundle.min.js' ), $src );
    }
    return $src;
}, 10, 2 );
